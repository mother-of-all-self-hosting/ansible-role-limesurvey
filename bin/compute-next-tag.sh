...
#
# Tags look like `v<LimeSurvey version>-<release>`, which is what this
# repository has always published (v3.28.16-0 ... v7.0.11-0):
#
# - if defaults/main.yml points at a LimeSurvey version that has never been
#   released, the release counter restarts at 0 (`v7.0.12-0`)
# - otherwise the counter is incremented (`v7.0.12-1`), but only if something
...
# to 7" subject for a version and published a `v7-0` tag off it, while
# defaults/main.yml said 7.0.x all along.
...
# Anchored on `limesurvey_version:` so that neither a commented-out example nor
# `limesurvey_container_image_tag`, which is derived from it, can be mistaken
# for it.
version="$(sed -nE 's|^limesurvey_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the LimeSurvey version from $defaults_path"
	exit 1
fi

# LimeSurvey's own version is carried without a leading `v` (the `v` lives in
# the tag), but tolerate one so that a future change of convention does not
